...
internal class Program {
    private static async Task Main(string[] args) {
        ...
        if (config.dbFolder == null) throw new Exception("dbFolder is required");
        ...
        var ferrumDb =
            new DatabaseEngine(new DatabaseEngineConfig { databasePath = config.dbFolder }, ioEngine);
        ...
        new API(ferrumDb, transactionEngine, queryEngine,
            new APIConfig {
                ip = config.ip,
                port = config.port,
    ...
    private static void printHelp() {
        ...
        Console.WriteLine(
            "  --garbage-collector-aggressiveness=<aggressiveness> Value from 0 to 100, 0 disables the garbage collector, 100 collects immediately as soon as any data is deleted");
        Console.WriteLine(
            "  --preloaded-collections=<collections> Collections that should be preloaded on startup and kept in memory");
        Console.WriteLine(
            "  --collection-idle-timeout=<timeout> The amount of time in seconds a collection can be idle before being unloaded from memory, 0 means never unload. Collections listed in preloadedCollections are never unloaded");
        Console.WriteLine(
            "  --write-cache-size=<size> The amount of data that can be cached in memory before being written to disk in MB");
        Console.WriteLine(
            "  --read-cache-size=<size> The amount of data that can be kept in memory in case it is needed again in MB");
    ...
    private static Config loadConfig(string[] args) {
        ...
        if (File.Exists(configFilePath)) {
            var config = JsonConvert.DeserializeObject<Config>(File.ReadAllText("config.json")) ??
                         throw new Exception($"Config file at {configFilePath} is invalid");

            foreach (var arg in normalizedArgs) {
                if (arg.Key == "config") continue;
        ...
        throw new Exception(
            $"Config file not found at {configFilePath}. Please create a config.json file or pass the path to a config file with --config=<path>");
    }

...
public class TimeSeries {
    ...
    public TimeSeries(string path, long pos, string name, uint pageSize, Set? transactionSet) {
    ...
    private void initialize(Set? transactionSet) {
        ...
        this.writer = new BinaryWriter(File.Open(Path.Join(path, "records.timeseries"), FileMode.OpenOrCreate));
    ...
    public byte[][] getSerieEntries(string key) {
        long[] timestamps = getSerieTimestmaps(key);

        var result = new byte[timestamps.Length][];
        for (var i = 0; i < timestamps.Length; i++) {
            result[i] = get(key, timestamps[i]) ?? new byte[0];
        }
